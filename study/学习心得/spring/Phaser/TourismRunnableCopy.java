...
public class TourismRunnableCopy implements Runnable {
    Phaser phaser;
    Random random;
    /**
    ...
    public TourismRunnableCopy(Phaser phaser) {
        this.phaser = phaser;
    ...
    private boolean randomEvent() {
        ...
        if (r < 10) {
            ...
            for (int i = 0; i < friendNum; i++) {
                new Thread(new TourismRunnableCopy(phaser), name + "的朋友" + frientCount.getAndAdd(1) + "号").start();
            }
        } else if (r > 90) {
    ...
    public static void main(String[] args) {
        ...
        for (char ch : name.toCharArray()){
            tourismThread.add(new Thread(new TourismRunnableCopy(phaser), "小" + ch));
        }
