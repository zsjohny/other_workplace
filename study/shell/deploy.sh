#!/bin/bash 
#先下载代码
exportpath="/var/lib/jenkins/workspace/etongjin_user_dev"
branch_name="miscroservice_user"
...
pid=`sudo ssh root@$server /stock/java/bin/jps -l|grep $branch_name|awk '{print $1}'`
...
#启动服务
`sudo ssh root@$server "nohup /stock/java/bin/java -jar   -Xms512m -Xmx1024m  /tmp/$branch_name.jar  --spring.profiles.active=dev >/dev/null 2>&1 &"`
