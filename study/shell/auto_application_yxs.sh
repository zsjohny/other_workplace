...
source /etc/profile
PROFILE="test"
addressArr="127.0.0.1"
`mkdir /tmp/$PROFILE >/dev/null 2>&1`
git_commit_log_path="/tmp/$PROFILE/miscroserver_application_git_commit_log.log"
git_commit_new_log_path="/tmp/$PROFILE/miscroserver_application_git_commit_new_log.log"
all_servers="miscroservice_user"
git_url="git@gitlab.com:nessary/xiaoshi_java.git"
git_path="/opt/git"
branch_project_name="xiaoshi_java"
multlAddress="127.0.0.1"
...
do 
	...
	do 
		...
		echo  $service
			...
			echo  $service
			`\cp -f $git_path/$branch_project_name/test/src/main/resources/deploy/docker-compose.yml $exportpath/$branch_name/ `
			`\cp  -f $git_path/$branch_project_name/test/src/main/resources/deploy/Dockerfile-$PROFILE $exportpath/$branch_name/ `
			
			
			`mv $exportpath/$branch_name/Dockerfile-$PROFILE $exportpath/$branch_name/Dockerfile`
