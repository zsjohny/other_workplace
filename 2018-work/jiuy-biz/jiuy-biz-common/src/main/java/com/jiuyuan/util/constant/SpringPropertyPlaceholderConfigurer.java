...
public class SpringPropertyPlaceholderConfigurer {
    ...
    private static final Logger log = LoggerFactory.getLogger(SpringPropertyPlaceholderConfigurer.class);
     ...
     * @param visitedPlaceholders the placeholders that have already been visited during the current resolution attempt
     *        (used to detect circular references between placeholders). Only non-null if we're parsing a nested
     *        placeholder.
    ...
    protected String resolveSystemProperty(String key) {
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Could not access system property ''", key, e);
            }
            return null;
        }
    }
    ...
    public String getValue(String key, Properties props) {
        Set<String> visitedPlaceholders = new HashSet<String>();
        String propVal = resolvePlaceholder(key, props, this.systemPropertiesMode);
        if (propVal != null) {
            // Recursive invocation, parsing placeholders contained in the
            // previously resolved placeholder value.
            propVal = parseStringValue(propVal, props, visitedPlaceholders);
        } else {
            throw new RuntimeException("Could not resolve placeholder '" + key + "'");
        }
        visitedPlaceholders.remove(key);
        return propVal;
    }
}