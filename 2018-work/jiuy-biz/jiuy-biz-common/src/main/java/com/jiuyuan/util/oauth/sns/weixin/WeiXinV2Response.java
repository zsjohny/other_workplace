...
public class WeiXinV2Response<T> extends AbstractSnsResponse<T> {

    private static final Logger log = LoggerFactory.getLogger(WeiXinV2Response.class);

    private static final Properties messages = loadMessages();

    private static final String ERRCODE = "errcode";

    public WeiXinV2Response(CachedHttpResponse response) {
        super(response);
    }

    public WeiXinV2Response(WeiXinV2Response<?> response, T data) {
        super(response, data);
    }
    ...
    private static Properties loadMessages() {
        ...
        Class<?> clazz = WeiXinV2Response.class;
        try {
