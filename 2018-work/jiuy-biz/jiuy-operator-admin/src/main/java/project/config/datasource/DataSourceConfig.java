...
public class DataSourceConfig implements EnvironmentAware {
	...
	public MapperScannerConfigurer mappers() {
		...
		mapperScannerConfigurer.setBasePackage(
				"com.jiuy.operator.common.system.dao;com.jiuy.operator.common.system.persistence.dao;com.store.dao;com.jiuyuan.dao;com.jiuy.rb.mapper");
	...
	public MybatisSqlSessionFactoryBean sqlSessionFactory(DataSource dataSource, GlobalConfiguration globalConfig) {
		...
		Resource[] rebuild = ResKit.getClassPathResources("classpath*:/com/jiuy/rb/xml/**/*.xml");
		Resource[] monitor = ResKit.getClassPathResources("classpath*:*/**/mapper/productMonitor/**/*.xml");

		List<Resource> resourceList = new ArrayList<>(Arrays.asList(classPathResources));
		resourceList.addAll(new ArrayList<>(Arrays.asList(rebuild)));
		resourceList.addAll(new ArrayList<>(Arrays.asList(monitor)));
		Resource[] newResource = new Resource[resourceList.size()];
		for (int i=0;i<resourceList.size();i++) {
			newResource[i] = resourceList.get(i);
		}
		sqlSessionFactory.setMapperLocations(newResource);
