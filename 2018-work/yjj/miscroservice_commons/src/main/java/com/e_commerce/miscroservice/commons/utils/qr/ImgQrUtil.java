package com.e_commerce.miscroservice.commons.utils.qr;
 ...
 * @author lichunxi
 */
public class ImgQrUtil {

    private static Logger log = LoggerFactory.getLogger(ImgQrUtil.class);
    ...
    private static BufferedImage genBarcode(String content, int width, int height, String srcImagePath)
        ...
        return doGenImg(content, width, height, srcPixels);
    }

    private static BufferedImage genBarcode(String content, int width, int height, InputStream inputStream)
            throws WriterException, IOException {
        // 读取源图像
        BufferedImage scaleImage = scale(inputStream, IMAGE_WIDTH, IMAGE_HEIGHT, true);
        int[][] srcPixels = new int[IMAGE_WIDTH][IMAGE_HEIGHT];
        for (int i = 0; i < scaleImage.getWidth(); i++) {
            for (int j = 0; j < scaleImage.getHeight(); j++) {
                srcPixels[i][j] = scaleImage.getRGB(i, j);
    ...
    private static BufferedImage doGenImg(String content, int width, int height, int[][] srcPixels) throws WriterException {
     ...
     * 生成上方带文字的二维码
     */
    public static File createQrWithFontsAbove(QrImage para) {
        File file=null;
        try {
            file = new File (para.getFileOutputPath ());
            // 首先生成二维码图片
            BufferedImage qrImage;
            if (para.getQrIconFilePath() == null) {
                qrImage = genBarcode(para.getQrContent(), para.getQrWidth(), para.getQrHeight(), para.getQrIconFileStream());
            } else {
                qrImage = genBarcode(para.getQrContent(), para.getQrWidth(), para.getQrHeight(), para.getQrIconFilePath());
            }
    ...
    public static void main(String[] args) {
        String path = "miscroservice_commons/";
        String imgName = path + "baby.jpg";
        System.out.println("imgName = " + imgName);
        for (int i = 1; i <= 1; i++) {
            QrImage para = new QrImage.Builder()
                    .setFileOutputPath(path + "qr.jpg")
                    .setQrContent("http://www.baidu.com?a=" + "123")
                    .setQrHeight(300)
                    .setQrWidth(300)
//                    .setQrIconFilePath(imgName)
                    .setQrIconFileStream(FileUtils.yjjLog())
                    .setTopWrodHeight(100)
                    .setWordContent("")
                    .setWordSize(18).build();
            ImgQrUtil.createQrWithFontsAbove(para);
        }
//        ImgQrUtil.createSimpleQr ("哈哈", 100, 300, path+"qr2.jpg");
    }
}
