...
var app = getApp()
var url = constant.devUrl + '/miniapp/homepage/navigationProductList.json';
/**
 * 上拉加载更多请求函数
 */
var pageNum = 2;
function loadMoreList(that, url, memberId, tagId){
 ...
 * 初始请求数据
 */
var initData = function (that, memberId, sessionId, tagId){
   var store_Id = wx.getStorageSync('storeId');
   dialog.loading();
   var sendData = {
      memberId: memberId, //会员id
   ...
   var sign = util.MD5(util.paramConcat(sendData));
   console.log("标签商品列表提交的数据：",sendData);
    wx.request({
      url: url, //接口地址
      data: sendData,
      header: {
...
var loginGetUserData = function (that) {
  ...
  app.loginAuthorizeFun(0, function (id) {
    ...
    initData(that, id, sessionId, that.data.tagId)
  })
}
var timer;
Page({
  ...
  onReachBottom: function () {
    ...
    if (moreComplete) {
      loadMoreList(that, url, memberId, tagId)
    }
  },
  //跳转至商品详情页面
