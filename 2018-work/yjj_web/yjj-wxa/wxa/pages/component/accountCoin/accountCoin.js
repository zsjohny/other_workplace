...
var dialog = require("../../../utils/dialog"),
    ...
    data = require("../accountAmount/data");
var app = getApp()
/**
 * 上拉加载更多通用请求函数
...
var getMoreList = function (that) {
  ...
  app.distributionApi.getBillDetails(param).then(res => {
    var data = res.data,
        accountDataArray = that.data.listData;
    //有数据
    if (data.length > 0) {
      that.setData({
...
var initData = function (that, memberId) {
  ...
  //金币数据
  app.distributionApi.getGoldBill().then(res => {
    var data = res.data;
    console.log("数据", data);
    that.setData({
      goldData: data,
...
Page({
  ...
  gotoAmountDetail: function (e) {
    var id = e.currentTarget.dataset.id,
        ...
        url = '../incomeDetail/incomeDetail?id=' + id + "&flag=coin" + "&inOutType=" + inOutType;
