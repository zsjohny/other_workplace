...
public class PDFBuilder extends PdfPageEventHelper {
    ...
    private static Log logger = Log.getInstance(PDFBuilder.class);
    ...
    private void initFront(){
        ...
        try {
            if (bf == null) {
                ...
                String fontPath =classpath.replaceAll("build/classes/main|out/production", "src/main")+"/"+fontFileName;
                //创建基础字体
                bf = BaseFont.createFont(fontPath,BaseFont.IDENTITY_H,BaseFont.EMBEDDED);
            }
            if (fontDetail == null) {
                fontDetail = new Font(bf, fontSize, Font.NORMAL);// 数据体字体
                logger.info("PDF文档字体初始化完成!");
            }
        } catch (DocumentException e) {
            logger.error("字体初始化失败{}", ExceptionUtils.getFullStackTrace(e));
