...
<c:if test="${authorLevel !=2}">
    <c:if test="${!empty pageName && pageName==false}">
        <button type="button" class="btn btn-success" id="shapeRiskExamineNew">新增</button>
    </c:if>
</c:if>
<input type="hidden" id="page" value="${page}"/>
<c:if test="${empty pageName}">
    <button type="button" class="btn btn-info" name="backid" id="shapeRiskExamineEditBackid">返回列表</button>
</c:if>
<c:if test="${!empty total && total !=0}">
    <button type="button" class="btn btn-info" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=0&className=shapeRiskExamine&excelName=大变形风险源调查">本页下载</a>
    </button>
    <button type="button" class="btn btn-warning" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=1&className=shapeRiskExamine&tunnelNumber=${params}&excelName=大变形风险源调查全部">全部下载</a>
    </button>
</c:if>
...
<c:if test="${total==1}">
    ...
    <form class="definewidth m20">
        <table class="table table-bordered table-hover definewidth m10">
            ...
            <tr>
                <td width="10%" class="tableleft">特殊岩土</td>
                <td><input type="text" name="specialRock" value="${baseDomain.specialRock}"/></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-primary" type="button" id="shapeRiskExamineEditSave">修改
                    </button>
                    <button type="button" class="btn btn-success" type="button"><a
                            href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${baseDomain.id}&tunnelNumber=${tunnelNumber}&className=shapeRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
                    </button>
        ...
        <if test="${!empty list}">
            ...
            <c:forEach items="${list}" var="list" varStatus="status">
                <tr>
                    ...
                    <c:if test="${authorLevel !=2}">
                        <td>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=shapeRiskExamine&page=${page}&pageType=Edit&pageName=${pageName}&params=${params}">编辑</a>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=shapeRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
...
<script>
    ...
    function forward(id) {
        ...
        location.href = " /tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?className=shapeRiskExamine&tunnelNumber=" + number + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&params=" + $("#params").val().trim();
    ...
    function del(id) {
        ...
        if (confirm("确定要删除吗？")) {
            ...
            var page = $("#page").val();
            window.location.href = "/tunnel/tunnelInfoTotal/removerTunnelInfoTotal.do?id=" + id + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&tunnelNumber=" + number + "&className=shapeRiskExamine&params=" + $("#params").val();
    ...
    $("#shapeRiskExamineEditBackid").click(function () {
    ...
    //添加
    $("#shapeRiskExamineNew").click(function () {


        location.href = "/tunnel/login/againMenu.do?path=tunnelInfoTotal/shapeRiskExamineAdd&params=" + $("#params").val().trim();
    });
    //修改
    $('#shapeRiskExamineEditSave').click(function () {
        ...
        var specialRock = $("input[name='specialRock']").val();
        var tunnelLine = $("input[name='tunnelLine']").val();
        var arroundEnviroment = $("input[name='arroundEnviroment']").val();

        $.post("/tunnel/tunnelInfoTotal/modifyTunnelInfoTotal.do", "className=shapeRiskExamine&id=" + id + "&tunnelNumber=" + tunnelNumber + "&riskSourceExists=" + riskSourceExists
                + "&riskHappendProbalility=" + riskHappendProbalility + "&riskDecribe=" + riskDecribe + "&startCourse=" + startCourse + "&endCourse=" + endCourse + "&generalSituation=" + generalSituation + "&geologicalStructure=" + geologicalStructure + "&undergroundWater=" + undergroundWater + "&badGeological=" + badGeological + "&specialRock=" + specialRock + "&tunnelLine=" + tunnelLine + "&arroundEnviroment=" + arroundEnviroment, function (data) {
