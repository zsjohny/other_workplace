...
<c:if test="${authorLevel !=2}">
    <c:if test="${!empty pageName && pageName==false}">
        <button type="button" class="btn btn-success" id="trafficAccidentRiskExamineNew">新增</button>
    </c:if>
</c:if>
<input type="hidden" id="page" value="${page}"/>
<c:if test="${empty pageName}">
    <button type="button" class="btn btn-info" name="backid" id="trafficAccidentRiskExamineEditBackid">返回列表</button>
</c:if>
<c:if test="${!empty total && total !=0}">
    <button type="button" class="btn btn-info" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=0&className=trafficAccidentRiskExamine&excelName=交通事故风险源调查">本页下载</a>
    </button>
    <button type="button" class="btn btn-warning" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=1&className=trafficAccidentRiskExamine&tunnelNumber=${params}&excelName=交通事故风险源调查全部">全部下载</a>
    </button>
</c:if>
    ...
    <form class="definewidth m20">
        <table class="table table-bordered table-hover definewidth m10">
            ...
            <tr>
                ...
                <td><input type="text" name="tunnelLength" value="${baseDomain.tunnelLength}"/></td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-primary" type="button" id="trafficAccidentRiskExamineEditSave">
                        修改
                    </button>
                    <button type="button" class="btn btn-success" type="button"><a
                            href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${baseDomain.id}&tunnelNumber=${tunnelNumber}&className=trafficAccidentRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
                    </button>
        ...
        <if test="${!empty list}">
            ...
            <c:forEach items="${list}" var="list" varStatus="status">
                <tr>
                    ...
                    <td>${list.parallelLine}</td>
                    <td>${list.verticalAlignment}</td>
                    ...
                    <td>${list.tunnelLength}</td>


                    <c:if test="${authorLevel !=2}">
                        <td>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=trafficAccidentRiskExamine&page=${page}&pageType=Edit&pageName=${pageName}&params=${params}">编辑</a>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=trafficAccidentRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
...
<script>
    ...
    function forward(id) {
        ...
        location.href = " /tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?className=trafficAccidentRiskExamine&tunnelNumber=" + number + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&params=" + $("#params").val().trim();
    ...
    function del(id) {
        ...
        if (confirm("确定要删除吗？")) {
            ...
            var page = $("#page").val();
            window.location.href = "/tunnel/tunnelInfoTotal/removerTunnelInfoTotal.do?id=" + id + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&tunnelNumber=" + number + "&className=trafficAccidentRiskExamine&params=" + $("#params").val();
    ...
    //添加
    $("#trafficAccidentRiskExamineNew").click(function () {


        location.href = "/tunnel/login/againMenu.do?path=tunnelInfoTotal/trafficAccidentRiskExamineAdd&params=" + $("#params").val().trim();
    });
    ...
    $('#trafficAccidentRiskExamineEditSave').click(function () {
        ...
        $.post("/tunnel/tunnelInfoTotal/modifyTunnelInfoTotal.do", "className=trafficAccidentRiskExamine&id=" + id + "&tunnelNumber=" + tunnelNumber + "&riskSourceExists=" + riskSourceExists
                + "&riskHappendProbalility=" + riskHappendProbalility + "&riskDecribe=" + riskDecribe + "&parallelLine=" + parallelLine + "&verticalAlignment=" + verticalAlignment + "&trafficRadio=" + trafficRadio + "&truckScaleRadio=" + truckScaleRadio + "&operationalVentilationScheme=" + operationalVentilationScheme + "&fileRecue=" + fileRecue + "&monitorPlan=" + monitorPlan + "&lightPlan=" + lightPlan + "&tunnelLength=" + tunnelLength, function (data) {
