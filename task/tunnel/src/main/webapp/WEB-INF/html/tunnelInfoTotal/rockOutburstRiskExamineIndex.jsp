...
<c:if test="${authorLevel !=2}">
    <c:if test="${!empty pageName && pageName==false}">
        <button type="button" class="btn btn-success" id="rockOutburstRiskExamineNew">新增</button>
    </c:if>
</c:if>
<input type="hidden" id="page" value="${page}"/>
<c:if test="${empty pageName}">
    <button type="button" class="btn btn-info" name="backid" id="rockOutburstRiskExamineEditBackid">返回列表</button>
</c:if>
<c:if test="${!empty total && total !=0}">
    <button type="button" class="btn btn-info" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=0&className=rockOutburstRiskExamine&excelName=岩爆风险源调查">本页下载</a>
    </button>
    <button type="button" class="btn btn-warning" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=1&className=rockOutburstRiskExamine&tunnelNumber=${params}&excelName=岩爆风险源调查全部">全部下载</a>
    </button>
</c:if>
    ...
    <form class="definewidth m20">
        <table class="table table-bordered table-hover definewidth m10">
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-primary" type="button" id="rockOutburstRiskExamineEditSave">
                        修改
                    </button>
                    <button type="button" class="btn btn-success" type="button"><a
                            href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${baseDomain.id}&tunnelNumber=${tunnelNumber}&className=rockOutburstRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
                    </button>
        ...
        <if test="${!empty list}">
            ...
            <c:forEach items="${list}" var="list" varStatus="status">
                <tr>
                    ...
                    <td>${list.uniaxialSaturatedCompressiveStrengthOfRock}</td>
                    <td>${list.geologicalStructure}</td>
                    <td>${list.tunnelLine}</td>
                    <td>${list.maxStrain}</td>
                    <td>${list.tunnelAxesAndMaxStrain}</td>


                    <c:if test="${authorLevel !=2}">
                        <td>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=rockOutburstRiskExamine&page=${page}&pageType=Edit&pageName=${pageName}&params=${params}">编辑</a>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=rockOutburstRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
...
<script>
    ...
    function forward(id) {
        ...
        location.href = " /tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?className=rockOutburstRiskExamine&tunnelNumber=" + number + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&params=" + $("#params").val().trim();
    ...
    function del(id) {
        ...
        if (confirm("确定要删除吗？")) {
            ...
            var page = $("#page").val();
            window.location.href = "/tunnel/tunnelInfoTotal/removerTunnelInfoTotal.do?id=" + id + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&tunnelNumber=" + number + "&className=rockOutburstRiskExamine&params=" + $("#params").val();
    ...
    //添加
    $("#rockOutburstRiskExamineNew").click(function () {


        location.href = "/tunnel/login/againMenu.do?path=tunnelInfoTotal/rockOutburstRiskExamineAdd&params=" + $("#params").val().trim();
    });
    //修改
