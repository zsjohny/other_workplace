...
<c:if test="${authorLevel !=2}">
    <c:if test="${!empty pageName && pageName==false}">
        <button type="button" class="btn btn-success" id="surgeMudRiskExamineNew">新增</button>
    </c:if>
</c:if>
<input type="hidden" id="page" value="${page}"/>
<c:if test="${empty pageName}">
    <button type="button" class="btn btn-info" name="backid" id="surgeMudRiskExamineEditBackid">返回列表</button>
</c:if>
<c:if test="${!empty total && total !=0}">
    <button type="button" class="btn btn-info" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=0&className=surgeMudRiskExamine&excelName=突水涌泥风险源调查">本页下载</a>
    </button>
    <button type="button" class="btn btn-warning" type="button">
        <a href="/tunnel/login/downloadExcel.do?downloadType=1&className=surgeMudRiskExamine&tunnelNumber=${params}&excelName=突水涌泥风险源调查全部">全部下载</a>
    </button>
</c:if>
    ...
    <form class="definewidth m20">
        <table class="table table-bordered table-hover definewidth m10">
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-primary" type="button" id="surgeMudRiskExamineEditSave">修改
                    </button>
                    <button type="button" class="btn btn-success" type="button"><a
                            href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${baseDomain.id}&tunnelNumber=${tunnelNumber}&className=surgeMudRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
                    </button>
        ...
        <if test="${!empty list}">
            ...
            <c:forEach items="${list}" var="list" varStatus="status">
                <tr>
                    ...
                    <c:if test="${authorLevel !=2}">
                        <td>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=surgeMudRiskExamine&page=${page}&pageType=Edit&pageName=${pageName}&params=${params}">编辑</a>
                            <a href="/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?id=${list.id}&tunnelNumber=${tunnelNumber}&className=surgeMudRiskExamine&page=${page}&pageType=Detail&pageName=${pageName}&params=${params}">详情</a>
...
<script>
    ...
    function forward(id) {
        ...
        location.href = " /tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?className=surgeMudRiskExamine&tunnelNumber=" + number + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&params=" + $("#params").val().trim();
    ...
    function del(id) {
        ...
        if (confirm("确定要删除吗？")) {
            ...
            var page = $("#page").val();
            window.location.href = "/tunnel/tunnelInfoTotal/removerTunnelInfoTotal.do?id=" + id + "&page=" + page + "&pageName=" + $("#pageName").val().trim() + "&tunnelNumber=" + number + "&className=surgeMudRiskExamine&params=" + $("#params").val();
    ...
    //添加
    $("#surgeMudRiskExamineNew").click(function () {


        location.href = "/tunnel/login/againMenu.do?path=tunnelInfoTotal/surgeMudRiskExamineAdd&params=" + $("#params").val().trim();
    });
