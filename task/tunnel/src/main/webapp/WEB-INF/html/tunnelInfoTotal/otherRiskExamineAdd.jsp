...
<form class="definewidth m20">
    <table class="table table-bordered table-hover definewidth m10">
        ...
        <tr>
            ...
            <td><input type="text" name="riskDesc"/></td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <button type="button" class="btn btn-primary" type="button" id="otherRiskExamineAddSave">保存</button>
                &nbsp;&nbsp;
                <button type="button" class="btn btn-success" name="backid" id="otherRiskExamineAddBackid">返回列表</button>
            </td>
        </tr>
    </table>
</form>
...
<script>
    $(function () {
        ...
        $('#otherRiskExamineAddSave').click(function () {
            ...
            $.post("/tunnel/tunnelInfoTotal/modifyTunnelInfoTotal.do", "className=otherRiskExamine&tunnelNumber=" + tunnelNumber + "&riskSourceExists=" + riskSourceExists
                    + "&riskHappendProbalility=" + riskHappendProbalility + "&riskDecribe=" + riskDecribe + "&location=" + location + "&riskDesc=" + riskDesc, function (data) {

                if (data == true) {
                    $("#otherRiskExamineAddBackid").click();
                } else {
                    $("input[name='riskDesc']").val("");

                    $("input[name='riskDesc']").attr("placeholder", "新增失败,错误原因联系技术!!");
        ...
        $("#otherRiskExamineAddBackid").click(function () {
            var params = $("#params").val();
            //判断是否是页面选择进来的还是从原始页面进来的
            if (params.split(":")[2] == '1') {

                location.href = '/tunnel/tunnelInfoTotal/findTunnelInfoTotal.do?pageName=false&className=otherRiskExamine&tunnelNumber=' + params.split(":")[1].trim() + '&params=' + params.split(":")[0].trim() + ":" + params.split(":")[1].trim() + ':1'
                return;
            }
